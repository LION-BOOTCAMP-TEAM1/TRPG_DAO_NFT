...
const CharacterStat = () => {
  ...
  return (
    <div className="p-4 rounded shadow flex flex-col items-center gap-2 text-sm font-medium w-full">
      {/* 상단: 캐릭터 정보 */}
      <div className="flex w-full">
        ...
        <div className="ml-2 flex flex-col justify-between">
          {/* 클래스 */}
          <div className="bg-white px-2 py-1 rounded text-xs w-fit">
            {myNFTs.character?.class}
          </div>

          {/* 이름 */}
          <div className="bg-white px-2 py-1 rounded text-xs w-fit">
            {myNFTs.character?.name}
          </div>
          ...
          <div className="grid grid-cols-4 gap-1 mt-2">
            ...
            {Array.from({
              length:
                (myNFTs.character?.stat?.HP ?? 0) - myNFTs.stats.HP > 0
                  ? (myNFTs.character?.stat?.HP ?? 0) - myNFTs.stats.HP
                  : 0,
            }).map((_, idx) => (
              <div
                key={`extra-${idx}`}
            ...
            {Array.from({
              length:
                (myNFTs.character?.stat?.MT ?? 0) - myNFTs.stats.MT > 0
                  ? (myNFTs.character?.stat?.MT ?? 0) - myNFTs.stats.MT
                  : 0,
            }).map((_, idx) => (
              <div
                key={`extra-${idx}`}
