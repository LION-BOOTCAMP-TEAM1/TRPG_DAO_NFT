      ...
      <div className="grid grid-cols-2 gap-8 h-[90%]">
        ...
        <div className="flex flex-col h-full overflow-y-auto">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="bg-zinc-700 px-3 py-1 rounded text-sm hover:bg-zinc-600 cursor-pointer"
              >
                가격순{sortPriceAsc ? '⬆️' : '⬇️'}
                </button>
              <button onClick={() => setSortRarityAsc((v) => !v )} className="bg-zinc-700 px-3 py-1 rounded text-sm hover:bg-zinc-600 cursor-pointer">등급순{sortRarityAsc ? '⬆️' : '⬇️'}</button>
            </div>
          </div>
          ...
          <div className="space-y-3 overflow-y-auto pr-2 flex-1 scrollbar-thin scrollbar-thumb-zinc-600 scrollbar-track-transparent pb-6">
            ...
            {marketItems.map(({ id, item, seller, price }, index) => (
              <MarketItem key={index} id={id} item={item} seller={seller} price={price} refresh={refresh} />
            ))}
          </div>
        </div>
